...
const Tablets = () => {
  ...
  useEffect(() => {
    axios
      .get("https://dummyjson.com/products/category/tablets")
      .then((response) => {
    ...
    if (isFavorite) {
      ...
      dispatch(
        addToFavorites({
          ...
          title: product.title,
          price: product.price,
          image: product.thumbnail || (product.images?.length ? product.images[0] : ""),
        })
      );
    }
  };
        ...
        {products.map((product) => {
          ...
          return (
            <Col key={product.id} xs={12} sm={6} md={4} lg={3}>
              <Card className="shadow-sm border-0 product-card position-relative">
                <FaStar
